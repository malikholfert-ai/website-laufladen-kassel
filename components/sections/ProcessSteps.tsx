...
export function ProcessSteps() {
  return (
    <div className="space-y-0">
      {steps.map(({ nr, title, description, aside }, index) => (
        <div
          key={nr}
          className="grid grid-cols-[48px_1fr] lg:grid-cols-[64px_1fr_220px] gap-5 lg:gap-8 py-8 border-b border-brand-tint last:border-0"
        >
          {/* Nummer */}
          <div className="pt-1">
            <span
              className="block text-2xl lg:text-3xl leading-none text-brand-sky"
          ...
          {/* Inhalt */}
          <div>
            <h3 className="font-semibold text-brand-navy font-sans text-[17px] mb-2">
              {title}
            </h3>
          ...
          <div className="hidden lg:flex items-start pt-1">
            <span className="inline-block px-3 py-1.5 bg-brand-bg rounded-lg text-brand-muted text-xs font-sans leading-snug">
              {aside}
            </span>
          </div>
        </div>
