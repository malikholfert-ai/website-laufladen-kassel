...
function Field({ label, id, error, required = false, children }: FieldProps) {
  return (
    <div className="flex flex-col gap-1.5">
      <label
        htmlFor={id}
        className="text-sm font-semibold text-brand-navy font-sans"
      >
        {label}
...
const inputNormal = "border-gray-200 hover:border-brand-sky";
...
export function BookingForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (result.success) {
      ...
      // Feld-spezifische Fehler vom Server zurücksetzen
      if (result.errors) {
    ...
    return (
      <div className="bg-brand-cta-light border border-brand-cta/20 rounded-2xl p-10 text-center">
        ...
        <button
          ...
          className="mt-6 text-sm text-brand-blue hover:text-brand-navy font-medium transition-colors"
        >
  ...
  return (
    <form
      ...
      noValidate
      aria-label="Laufanalyse-Terminanfrage"
    >
      ...
      {serverError && (
        <div
          className="mt-5 flex items-start gap-2.5 p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm"
          role="alert"
        >
